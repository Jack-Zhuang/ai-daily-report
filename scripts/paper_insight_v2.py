#!/usr/bin/env python3
"""
论文解读生成器 V2 - 内容驱动、质量优先

核心理念：
1. 内容驱动：有什么内容就展示什么，没有就不展示
2. 质量优先：宁可少而精，不要多而空
3. 论文特色：突出每篇论文的独特亮点

不再使用固定模板，而是根据论文内容动态构建页面结构。
"""
...
class PaperInsightV2:
    ...
    def generate(self, paper: dict) -> str:
        ...
        # 4. 智能分析论文内容
        print("  🤖 智能分析...")
        analysis = self._analyze_paper(pdf_content, paper)
    ...
    def _analyze_paper(self, content: str, paper: dict) -> Dict:
        """智能分析论文 - 提取有意义的内容"""
        ...
        prompt = f"""你是一位资深的学术论文解读专家。请深度分析以下论文，提取有意义的内容。
...
请严格按照以下 JSON 格式输出。注意：如果某项内容在论文中没有明确提及，请设为空字符串或空数组，不要编造。

{{
    "abstract": "论文核心贡献的中文解读（200-400字，必须包含具体方法和数值结果）",
    "key_insights": ["关键洞察1", "关键洞察2", "关键洞察3"],
    "background": "研究背景和动机（150-250字，说明为什么需要这项研究）",
    "method": "方法概述（200-350字，说明核心思路和技术路线）",
    "innovations": [
    ...
    "experiments": {{
        "datasets": ["数据集名称及用途"],
        "results": ["具体实验结果（必须包含数值，如：在XX数据集上准确率提升X%）"],
        "comparisons": ["与基线方法的对比结果"]
    }},
    "findings": ["主要发现1", "主要发现2"],
    "limitations": "研究的局限性（如果论文中有提及）",
    "future_work": "未来研究方向（如果论文中有提及）",
    "applications": "潜在应用场景",
...
4. 只输出 JSON，不要其他内容"""
    ...
    def _build_page(self, paper: dict, analysis: Dict, figures: List, arxiv_id: str) -> str:
        ...
        if limitations or future_work or applications:
            sections.append(self._section_discussion(limitations, future_work, applications))
    ...
    def _section_discussion(self, limitations: str, future_work: str, applications: str) -> str:
        """讨论部分"""
        items = []
        if limitations:
            items.append(f'<div class="discussion-item"><h4>局限性</h4><p>{limitations}</p></div>')
        if future_work:
