...
class ArxivCollector:
    ...
    def run(self):
        ...
        if papers:
            ...
            # ⚠️ 采集失败 — 使用缓存数据兜底
            print(f"\n⚠️  arXiv 论文采集失败（0篇），尝试从缓存恢复...")
            cache_file = self.cache_dir / "arxiv_papers.json"
            if cache_file.exists():
                try:
                    cache_data = json.loads(cache_file.read_text())
                    cached_papers = cache_data.get('items', [])
                    cached_date = cache_data.get('last_update', '')
                    if cached_papers:
                        print(f"\n   📦 使用缓存数据（{cached_date}，共 {len(cached_papers)} 篇）")
                        self.save_papers(cached_papers)
                        print(f"   ⚠️ 注意：这些是缓存数据，不是最新论文")
                        return cached_papers, False
                except Exception as e:
                    print(f"   ❌ 缓存读取失败: {e}")
            
            # 缓存也没有，无法恢复
            print(f"\n   ❌ 缓存也不可用，今日 arXiv 板块将为空")
            print(f"   🔔 请检查 arXiv API 状态: https://export.arxiv.org/api/query")
            self.save_papers([])
            return [], False
...
if __name__ == "__main__":
    ...
    papers, ok = collector.run()
    
    # 采集失败时退出码非 0，便于上层脚本感知
    if not ok:
        print("\n⚠️  arXiv 论文采集未完成（使用缓存或空数据），请关注")
        sys.exit(2)
    else:
        print("\n✅ arXiv 论文采集完成")
